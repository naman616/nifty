...
def generate_suggestions(df):
    ...
    if bullish_count >= 3:
        ...
        suggestions.append("[*] Consider buying near support level: ${:,.2f}".format(support))
        suggestions.append("[>] Target resistance: ${:,.2f}".format(resistance))
        ...
        suggestions.append("[>] Watch support level: ${:,.2f}".format(support))
        risk_level = "High"
    else:
        action = "HOLD"
        suggestions.append("[-] Mixed signals - Market in consolidation")
        suggestions.append("[*] Wait for clearer trend confirmation")
        suggestions.append("[>] Range: ${:,.2f} - ${:,.2f}".format(support, resistance))
        risk_level = "Medium"
...
def print_analysis_report(df, analysis):
    ...
    print("\n" + "="*70)
    print("BITCOIN (BTC) TREND ANALYSIS REPORT")
    print("="*70)
    print("Analysis Date: {}".format(datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
    print("Current Price: ${:,.2f}".format(analysis['current_price']))
    print("Previous Close: ${:,.2f}".format(float(prev['Close'])))
    
    change = analysis['current_price'] - float(prev['Close'])
    change_pct = (change / float(prev['Close'])) * 100
    print("Change: ${:,.2f} ({:+.2f}%)".format(change, change_pct))
    ...
    print("SMA 20: ${:,.2f}".format(float(latest['SMA_20'])))
    print("SMA 50: ${:,.2f}".format(float(latest['SMA_50'])))
    
    print("\n" + "-"*70)
    print("SUPPORT & RESISTANCE")
    print("-"*70)
    print("Resistance: ${:,.2f}".format(analysis['resistance']))
    print("Current:    ${:,.2f}".format(analysis['current_price']))
    print("Support:    ${:,.2f}".format(analysis['support']))
    ...
    print("Cryptocurrency is highly volatile and risky.")
...
def main():
    ...
    print("Starting Bitcoin Trend Analysis...\n")
    ...
    output_file = 'bitcoin_analysis.csv'
